...
interface CsrFile;
    ...
    method Action incExecInstCnt(InstMissCntType inst);
endinterface
...
module mkCsrFile(CsrFile);
    ...
    Reg#(Data) numJExec  <- mkConfigReg(0);
    Reg#(Data) numJrExec <- mkConfigReg(0);
    Reg#(Data) numBrExec <- mkConfigReg(0);
    ...
    endmethod
     ...
     method Action incExecInstCnt(InstMissCntType inst);
        case (inst)
            J:  numJExec  <= numJExec  + 1;
            Jr: numJrExec <= numJrExec + 1;
            Br: numBrExec <= numBrExec + 1;
        endcase
    endmethod
