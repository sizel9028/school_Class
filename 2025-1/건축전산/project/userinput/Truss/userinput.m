function dummy = userinput(myInput)
    ...
    switch parts{1}
        ...
        case 'set'
            ...
            checkNum = str2double(numDummy);
            checkSet = str2double(setNum);
            if any(isnan(checkNum)) && isnan(checkSet)
                disp('숫자 좌표가 아닙니다');
                return;
            end
            ...
            switch type
                case {'e','E'}
                    dummy = ["set","e",numDummy,setNum,];
                case {'a','A'}
                    dummy = ["set","a",numDummy,setNum];
                otherwise
                    disp('없는 타입입니다');
                    return;
            end
